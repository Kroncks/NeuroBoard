...
| Example | What it does | Configuration | Expected result |
|---|---|---|---|
| `1_LED` | Cycles the onboard LED through red, green, blue | The Neuroboard alone is sufficient. | 🔴 **red** → 🟢 **green** → 🔵 **blue**, 1s each, forever |
| `2_SD` | Initializes the SD card and writes `"bonsoir"` to `/test.txt` | SD card required | 🟠 **orange** if the card doesn't mount<br>🟢 **green** if the mount and the write succeed<br>🔴 **red** otherwise |
| `3_Test_init_camera` | Initializes the camera only | Camera required | 🟢 **green** if `initCamera()` succeeds<br>🔴 **red** otherwise |
| `4_Photo` | Initializes camera + SD, captures one frame, saves it to `/photo_<ms>.jpg` | Camera required<br>SD card required | 🟢 **green** if capture and save both succeed<br>🟣 **purple** if the camera init fails<br>🟠 **orange** if the SD card init fails<br>🔴 **red** if camera init and SD init fails, or the save fails |
| `5_Demo_complete` | Starts a Wi-Fi AP + web UI with live MJPEG stream, snapshot, and save-to-SD | ⚠️ **YOU MUST RENAME THE WIFI NETWORK IN THE CODE**<br> Camera required<br>SD card required | 🟢 **green** once everything is ready<br>🟣 **purple** if the camera init fails<br>🟠 **orange** if the SD card init fails<br>🔴 **red** if both fail |
| `12_OLED` | Runs a complete OLED demonstration with custom font, centered text, and basic graphics | OLED screen required<br>- SDA = 4<br>- SCL = 5 | 🟢 green when the OLED demo starts successfully<br>🔴 red if the OLED is not detected or initialization fails